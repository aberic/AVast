...
public class FileVast {
    ...
    public FileVast(Context context) {
        this.mContext = context;
    }

    /**
     * 创建文件夹 File
     * 首选SD卡上的目录,具体是指 /sdcard/Android/data/package_name/cache 目录
     *
     * @param uniqueName
     *         目录唯一名
     *
     * @return File（目录路径）
     */
    public void createFileDir(String uniqueName) {
        File file;
        /* 外部存储是否可用 */
        boolean externalStorageAvailable = Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED);
        final String createPath;// 创建路径
        if (externalStorageAvailable) {
            createPath = mContext.getExternalFilesDir(null).getPath();
            Log.d("FileVast", "createPath|true = " + createPath);
        } else {
            createPath = mContext.getFilesDir().getPath();
            Log.d("FileVast", "createPath|false = " + createPath);
        }
        file = new File(createPath + File.separator + uniqueName);
        if (!file.exists()) {// 判断本地文件夹是否存在,否则创建
            file.mkdirs();
        }
    }
     ...
     * @return 路径
     */
    public String getFileDir(String uniqueName) {
        File file;
        /* 外部存储是否可用 */
        boolean externalStorageAvailable = Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED);
